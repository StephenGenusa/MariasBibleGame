...
def write_png(path: pathlib.Path, size: int = 180) -> None:
    ...
    # Proportional to the canvas, so the 192 and 512 icons look like the 180 one.
    scale = max(1, size // 15)
...
MANIFEST = {
    "name": "Five Clues",
    "short_name": "Five Clues",
    "description": "Five clues about a Bible related character or topic, one at a time.",
    "start_url": "./",
    "scope": "./",
    "display": "standalone",
...
def main() -> None:
    ...
    (out.parent / "manifest.webmanifest").write_text(
        json.dumps(MANIFEST, indent=2) + "\n", encoding="utf-8")
    for size in (180, 192, 512):
        write_png(out.parent / f"icon-{size}.png", size)
    # Skip Jekyll on GitHub Pages so every file is served verbatim.
    (out.parent / ".nojekyll").write_text("", encoding="utf-8")
    print(f"built {out.relative_to(ROOT)} ({len(html):,} bytes) from {week_path.name}")
    print("wrote dist/sw.js, dist/manifest.webmanifest and the icons")
